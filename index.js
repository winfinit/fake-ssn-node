...
highgroup = highgroup.trim();

highgroup = highgroup.replace(/\* /g, "\t");
highgroup = highgroup.replace(/\*/g, '');
highgroup = highgroup.replace(/ \t/g, "\t");
highgroup = highgroup.replace(/\n/g, "\t");
highgroup = highgroup.replace(/  /g, "\t");

highgroup = highgroup.split("\t");

var cleangroup = [];

for (var index in highgroup) {
        var val = highgroup[index];
        if (val.trim() != '') {
                var temp = val.split(' ');
                cleangroup[Number(temp[0].trim())] = Number(temp[1].trim());
        }
}
highgroup = cleangroup;
console.log(JSON.stringify(highgroup));
...
exports.generate = function (state) {
        ...
        var ssn = sprintf("%03s-%02s-%04s", area, group, Math.floor(Math.random() * 9999));
        return ssn;
}
